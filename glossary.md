...
**Environmental physics coupling** (Session 7, refined Session 8) — The mechanism by which a macro-structure's own physical dynamics (e.g. airflow, thermal transport) redistribute the micro-scale stigmergic signals that guide building. The Mahadevan group's termite mound work (King, Ocko & Mahadevan, PNAS 2015; Ocko, Heyde & Mahadevan, PNAS 2019) shows this is what turns a passive accumulation into a trace→actor loop: the structure's physics becomes the feedback path. Session 8 refinement: this is the *specific* negative-feedback mechanism the crossing (H7) requires — the accumulated structure must introduce a transport dynamics absent at the deposit level. A 20-year modeling lineage (Deneubourg 1977 → Bonabeau 1997 → Ladley & Bullock 2004) all lack this coupling (deposited material has no influence on agent movement); sim06 inherited the same limitation. The minimal lumped form for sim07: a structure-sourced transport field with a mass threshold `M_c` (inert → active state transition).
