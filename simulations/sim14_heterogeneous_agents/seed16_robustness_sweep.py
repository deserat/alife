"""
16-seed robustness of shuffled iteration — does 8/8 full hold? Is the -0.019 gap
statistical or structural?

Queued-topic #182: Shuffled 90/50 achieves 8/8 full at 8 seeds. Does it hold at
16 seeds, or is 8/8 a small-sample effect (cf. Session 49's n=220 g=0.06 which
dropped 4/4 → 6/8)?

Queued-topic #183: The L/R gap under shuffle is -0.019 (90/50 slightly > 50/90).
Is this statistical (8 seeds, ±0.03 noise) or structural? At 16 seeds the noise
band shrinks by ~√2. If the gap vanishes, it is statistical; if it persists,
there is a structural asymmetry beyond processing order.

Design:
  3 configs (50/50, 50/90, 90/50) × 16 seeds × {2, 1} seeds (perturbed)
  + 16-seed unperturbed baseline
  = 3 × 16 × 2 + 16 × 2 = 128 runs
  ...
  16 seeds: original 8 + 8 new for statistical power
  [42, 123, 256, 999, 100, 777, 1337, 314159,
   2024, 8888, 555, 1111, 314, 271, 9999, 12345]

  The 8 original seeds allow direct comparison with Session 62's 8-seed results.
  The 8 new seeds test whether the 8/8 full at shuffled 90/50 is a small-sample
  effect.
...
RESULTS_PATH = os.path.join(OUTPUT_DIR, "seed16_robustness_sweep.json")
...
SEEDS_8_ORIG = [42, 123, 256, 999, 100, 777, 1337, 314159]
...
if __name__ == "__main__":
    ...
    n_runs = len(CONFIGS) * len(SEEDS_16) * 2 + len(SEEDS_16) * 2
    print(f"\n{'='*80}")
    print(f"  16-SEED ROBUSTNESS SWEEP — SHUFFLED ITERATION")
    print(f"  n={N_TERMITES} g={G_FORM} perturb_at={PERTURB_AT} iter_shuffle=True")
    print(f"  configs={len(CONFIGS)} seeds={len(SEEDS_16)} "
          f"× {{2, 1}} = {n_runs} runs")
    print(f"{'='*80}")
    ...
    # Unperturbed baseline (16 seeds)
    print(f"\n  Computing unperturbed baseline (shuffled, 16 seeds)...")
    unper_2 = []
    unper_1 = []
    for sd in SEEDS_16:
    ...
    su_baseline_16 = summarize(unper_2, unper_1, len(SEEDS_16))
    su_baseline_8 = summarize_subset(unper_2, unper_1, set(SEEDS_8_ORIG), SEEDS_16)
    print(f"\n  SHUFFLED BASELINE (16 seeds): l2={su_baseline_16['l2']} "
    ...
    print(f"  SHUFFLED BASELINE (8 orig):  l2={su_baseline_8['l2']} "
          f"coexist={su_baseline_8['coexist']} "
          f"stable={su_baseline_8['stable']} h7={su_baseline_8['h7']} "
          f"full={su_baseline_8['full']} cf={su_baseline_8['mean_cf']:.3f}")

    all_results["baseline"] = {
        "16seed": {"2seed": unper_2, "1seed": unper_1,
                    "summary": su_baseline_16},
        "8orig": {"summary": su_baseline_8},
    }
    ...
    for pf_left, pf_right, label in CONFIGS:
        ...
        sp_16 = summarize(per_2, per_1, len(SEEDS_16))
        sp_8 = summarize_subset(per_2, per_1, set(SEEDS_8_ORIG), SEEDS_16)

        print(f"\n  shuffled {label} PERT (16 seeds): l2={sp_16['l2']} "
              f"coexist={sp_16['coexist']} "
              f"stable={sp_16['stable']} h7={sp_16['h7']} "
              f"full={sp_16['full']} cf={sp_16['mean_cf']:.3f} "
              f"tot_rec={sp_16['mean_total_rec']:.3f} "
              f"1s_l2={sp_16['l2_1s']} cells={sp_16['mean_cells']}")
        print(f"  shuffled {label} PERT (8 orig):  l2={sp_8['l2']} "
              f"coexist={sp_8['coexist']} "
              f"stable={sp_8['stable']} h7={sp_8['h7']} "
              f"full={sp_8['full']} cf={sp_8['mean_cf']:.3f}")

        all_results[label] = {
            "pf_left": pf_left,
            "pf_right": pf_right,
            "perturbed": {
                "16seed": {"2seed": per_2, "1seed": per_1,
                            "summary": sp_16},
                "8orig": {"summary": sp_8},
            },
        }
    ...
    print(f"\n{'='*120}")
    print(f"  16-SEED ROBUSTNESS SWEEP SUMMARY (shuffled, n=350, g=0.01, 160x160, dual, focal 0.3, jit=10)")
    ...
    for pf_left, pf_right, label in CONFIGS:
        if label not in all_results:
            continue
        sp16 = all_results[label]["perturbed"]["16seed"]["summary"]
        sp8 = all_results[label]["perturbed"]["8orig"]["summary"]
        print(f"{label:>8s} | {'16':>3s} | "
              f"{sp16['h7']:>5s} {sp16['coexist']:>8s} {sp16['stable']:>7s} {sp16['full']:>5s} | "
              f"{sp16['mean_cf']:>6.3f} {sp16['mean_total_rec']:>8.3f} "
              f"{sp16['l2_1s']:>6s} {sp16['mean_cells']:>6d}")
        print(f"{label:>8s} | {'8':>3s} | "
              f"{sp8['h7']:>5s} {sp8['coexist']:>8s} {sp8['stable']:>7s} {sp8['full']:>5s} | "
              f"{sp8['mean_cf']:>6.3f}")

    # Key comparisons
    cf_50_50_16 = all_results["50_50"]["perturbed"]["16seed"]["summary"]["mean_cf"]
    cf_50_90_16 = all_results["50_90"]["perturbed"]["16seed"]["summary"]["mean_cf"]
    cf_90_50_16 = all_results["90_50"]["perturbed"]["16seed"]["summary"]["mean_cf"]
    cf_50_50_8 = all_results["50_50"]["perturbed"]["8orig"]["summary"]["mean_cf"]
    cf_50_90_8 = all_results["50_90"]["perturbed"]["8orig"]["summary"]["mean_cf"]
    cf_90_50_8 = all_results["90_50"]["perturbed"]["8orig"]["summary"]["mean_cf"]

    full_90_50_16 = all_results["90_50"]["perturbed"]["16seed"]["summary"]["full"]
    full_90_50_8 = all_results["90_50"]["perturbed"]["8orig"]["summary"]["full"]
    ...
    print(f"    8-seed:  full={full_90_50_8}  cf={cf_90_50_8:.3f}")
    print(f"    16-seed: full={full_90_50_16} cf={cf_90_50_16:.3f}")
    holds = full_90_50_16 == "8/8" or full_90_50_16 == "16/16"
    print(f"    Holds at 16: {'YES' if holds else 'NO'}")

    print(f"\n  #183: Is the -0.019 gap statistical or structural?")
    print(f"    8-seed:  50/90 cf={cf_50_90_8:.3f}  vs  90/50 cf={cf_90_50_8:.3f}  "
          f"gap={cf_50_90_8 - cf_90_50_8:+.3f}")
    print(f"    16-seed: 50/90 cf={cf_50_90_16:.3f}  vs  90/50 cf={cf_90_50_16:.3f}  "
          f"gap={cf_50_90_16 - cf_90_50_16:+.3f}")
    gap_8 = abs(cf_50_90_8 - cf_90_50_8)
    gap_16 = abs(cf_50_90_16 - cf_90_50_16)
    print(f"    Gap 8-seed:  {gap_8:.3f}")
    print(f"    Gap 16-seed: {gap_16:.3f}")
    if gap_16 < 0.01:
        print(f"    Conclusion: STATISTICAL (gap vanishes at 16 seeds)")
    elif gap_16 < gap_8 * 0.5:
        print(f"    Conclusion: MOSTLY STATISTICAL (gap shrinks >50%)")
    else:
        print(f"    Conclusion: STRUCTURAL (gap persists at 16 seeds)")

    print(f"\n    50/50 cf: 8-seed={cf_50_50_8:.3f}  16-seed={cf_50_50_16:.3f}")
    best_16 = max(cf_50_50_16, cf_50_90_16, cf_90_50_16)
    best_label = "50/50" if best_16 == cf_50_50_16 else ("50/90" if best_16 == cf_50_90_16 else "90/50")
    print(f"    Best config at 16 seeds: {best_label} (cf={best_16:.3f})")

    # Determinism verification
    print(f"\n  Determinism check...")
    p_det = make_params(shuffle=True)
    pf_l, pf_r, _ = CONFIGS[2]  # 90/50
    ...
    print(f"  shuffled 90/50 seed=42: cells={m1['cells']} vs {m2['cells']} "
