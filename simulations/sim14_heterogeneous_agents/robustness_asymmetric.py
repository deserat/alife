"""
8-seed robustness of bilateral perturbation — does 4/4 full hold at 8 seeds?

Queued-topic #173: Does bilateral 50/50's 4/4 full (cf=0.825) hold at 8 seeds?
Queued-topic #174: Is the L/R asymmetry (50/90 > 90/50) systematic or 4-seed noise?

Design:
  3 configs (50/50, 50/90, 90/50) × 8 seeds × {perturbed, unperturbed} × {2,1} seeds
...
RESULTS_PATH = os.path.join(OUTPUT_DIR, "robustness_asymmetric_sweep.json")
...
# Bilateral configs: (pf_left, pf_right, label)
CONFIGS = [
    (0.50, 0.50, "50_50"),
    (0.50, 0.90, "50_90"),
    (0.90, 0.50, "90_50"),
...
if __name__ == "__main__":
    ...
    n_runs = len(CONFIGS) * len(SEEDS) * 2 * 2
    print(f"\n{'='*80}")
    print(f"  8-SEED ROBUSTNESS: BILATERAL PERTURBATION SWEEP")
    ...
    # Pre-compute unperturbed baseline (8 seeds).
    print(f"\n  Pre-computing unperturbed baseline (8 seeds)...")
    unper_2_all = []
    unper_1_all = []
    for sd in SEEDS:
    ...
    print(f"\n  BASELINE (8 seeds): l2={su_baseline['l2']} coexist={su_baseline['coexist']} "
    ...
    print(f"\n{'='*120}")
    print(f"  8-SEED ROBUSTNESS SWEEP SUMMARY (n=350, g=0.01, 160x160, dual, focal 0.3, jit=10)")
    ...
    print(f"{'config':>8s} {'L%':>5s} {'R%':>5s} | "
          ...
          f"{'cf':>6s} {'tot_rec':>8s} {'1s_l2':>6s} {'1s_h7':>6s} {'cells':>6s}")
    print("-" * 120)
    for pf_left, pf_right, label in CONFIGS:
        ...
        print(f"{label:>8s} {pf_left*100:>4.0f}% {pf_right*100:>4.0f}% | "
              ...
              f"{sp['l2_1s']:>6s} {sp['h7_1s']:>6s} {sp['mean_cells']:>6d}")

    # Determinism verification
    print(f"\n  Determinism check...")
    for label in ["50_50", "50_90"]:
        pf_left, pf_right, _ = [(l, r, lbl) for l, r, lbl in CONFIGS
                                 if lbl == label][0]
