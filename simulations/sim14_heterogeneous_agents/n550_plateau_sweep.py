"""
n=550–600 ultra-high-density plateau sweep.

Queued-topics #157, #161 (top priority since Session 54).

The 1/√n (Laplace pressure) scaling has been confirmed from n=170 to n=500
(~3% to ~31% grid fill). The LSW "droplet dissolves" prediction (g* → 0 when
the structure fills the grid) has NOT been realized at any density tested.

At n=550–600 (~34–37% grid fill, ~8500–9500 cells), the 1/√n predicts
g*(550)≈0.005, g*(600)≈0.003. The 2D site percolation threshold is ~59%
fill — we are still far from it.

Questions:
1. Does g* ever hit zero, or does the 1/√n hold indefinitely?
2. Does the 1-seed structural guarantee continue to strengthen (was 0/4 at
   n=500 — perfect for the first time)?
3. Does the stability-density trade-off (30th mechanism: boundary over-splits
   larger structures) worsen at n=550–600?
...
SWEEP_PATH = os.path.join(SIM14_DIR, "output", "n550_plateau_sweep.json")
...
# Part 1: n=550, 600 at gains 0.003, 0.005, 0.01
# 1/√n predictions: g*(550)≈0.005, g*(600)≈0.003
PLATEAU_COMBOS = [
    (550, 0.003, 0.003, "n550_g003"),
    (550, 0.005, 0.005, "n550_g005"),
    (550, 0.01, 0.01, "n550_g010"),
    (600, 0.003, 0.003, "n600_g003"),
    (600, 0.005, 0.005, "n600_g005"),
    (600, 0.01, 0.01, "n600_g010"),
]

# Part 2: no-inhibition control at n=550 and n=600
NO_INHIB_COMBOS = [
    (550, 0.0, 0.0, "n550_g000_no_inhib"),
    (600, 0.0, 0.0, "n600_g000_no_inhib"),
]
...
def run_plateau_sweep():
    """Part 1: n=550–600 plateau sweep."""
    t0 = time.time()
    results = {"config": {
        "grid": GRID,
        "jitter": JITTER,
        "note": "Session 66: n=550-600 ultra-high-density plateau (#157, #161)",
        "sqrt_prediction": "g* = -0.95 + 15.2/sqrt(n) → "
                          "g*(550)≈0.005, g*(600)≈0.003",
        ...
        "grid_fill_at_n550": "~33% (n=550, ~8500 cells / 25600)",
        "grid_fill_at_n600": "~37% (n=600, ~9500 cells / 25600)",
    }}

    print(f"\n{'='*80}")
    print(f"  N550 PLATEAU: n=550, 600 at g=0.003-0.01 × 4 seeds × {{2,1}}")
    print(f"{'='*80}")
...
def run_no_inhib_sweep():
    ...
    results = {"note": "Part 2: no-inhibition control — does fragmentation "
                        "persist at n=550-600 without the boundary?"}

    print(f"\n{'='*80}")
    print(f"  NO-INHIBITION CONTROL: n=550, 600 at g=0 × 4 seeds × {{2,1}}")
    print(f"{'='*80}")
...
def run_sweep():
    ...
    # Summary table
    print(f"\n{'='*120}")
    print("  N550 PLATEAU SWEEP SUMMARY (160x160, dual, focal 0.3, jit=10)")
    print(f"{'='*120}")
