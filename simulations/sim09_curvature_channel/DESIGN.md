...
- [x] **Part 8** — Perturbation / self-repair experiment (the H7 acid test, applied to the curvature channel)
...
- 2026-08-01 — Part 8 implemented: added optional `perturb={"at": step, "frac": f}` to `run_condition` (minimal edit, default `perturb=None` keeps Part 6 behavior unchanged). At `perturb_at` (default `int(0.6*steps)`) a central square patch covering `perturb_frac` (default 0.25) of the grid area is zeroed in `field.material` (and `field.pheromone` for the baseline). `compute_metrics` gained a `pre_perturb_total` kwarg so each post-damage record carries `recovery = current_total_material / pre_perturb_total_material`; `summarize` adds `recovery_final`, `perturb_at`, `perturb_frac`. `cmd_run` runs the perturbation experiment for both conditions (seed=SEED) under a top-level `"perturbation"` key and prints a recovery comparison. Verification command passes; all selftests pass (Part 1–5 OK); Part 6 non-perturbed behavior unchanged. At default params the curvature channel recovers to 1.13× pre-damage total (it refills the 25% hole) and the baseline to 47.34× — but the baseline number is an artifact of unbounded material accumulation (the saturating deposit rule piles material without an erosion balance, so total_material grows monotonically ~47× from the early pre-damage sample), not genuine targeted repair. The honest reading: at default params the perturbation test does NOT separate the channels the way H7 predicts — the baseline's saturating rule refills volume trivially because it never stops growing. The separation the acid test needs (curvature recruits repair at the scar; baseline does not) requires the tuned parameter regime (lower nucleation, higher erosion) where mass saturates, the same regime Part 7 identified as needed for the crossing. Part 9 (viz+README) remains.
- 2026-08-02 — Part 9 implemented: wrote `visualize.html` (self-contained HTML5 Canvas, dark theme #0d1117, fetches results.json + optional output/sweep_data.json; four charts — structure-over-time, roughness+stability, deposits-on-convex criterion 3, perturbation recovery — plus a d-sweep phase-transition panel with crossed/retention/n_pillars, summary boxes, and a full result table) and `README.md` (real numbers from the default-param run: neither condition crosses at d=1.0, curvature grid-saturates to 10000 cells / baseline 4833, perturbation recovery 1.13× curvature vs 47.34× baseline-inflated, d-sweep finds no phase transition at default nucleation but tuned probes show the predicted consolidation direction). Verified via local http server: page/results/sweep all return 200, no console errors. All selftests still pass (Part 1–5 OK). sim09 is now fully complete (all 9 Parts [x]).
