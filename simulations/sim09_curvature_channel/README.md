...
sim09's result is now a **positive test of H7 with a control arm** — the corrected detector fires the crossing in the curvature channel and not in the baseline-pheromone control (same detector). H11's channel distinction (non-saturating action-channel vs saturating cue-channel) is the causal variable separating crossing from non-crossing, not just a directional correlate. The recruit half is load-bearing for the crossing; the limit half is load-bearing for morphology — so H11's "recruit as well as limit" refinement is half-supported. The next test isolates the halves (recruit-only d=0 vs limit-only no-curvature-routing). If the crossing fires only above the Facchini `d*` instability, sim09 unifies the directed-transport and non-saturating-inhibition candidates (queued-topic 58): curvature IS the minimal lumped form of directed geometry.
...
2. **Isolate the recruit and limit halves.** The crossing fires at d=0 (no smoothing), so the recruit half (curvature routing) is sufficient. A recruit-only condition (d=0) vs a limit-only condition (smoothing, no curvature routing) would determine whether the limit half contributes to the crossing verdict or only to morphology. This is the direct test of H11's "recruit as well as limit" refinement.
3. **Spatially-targeted recovery metric.** Measure recovery in the damaged patch specifically, not grid-wide, to make the perturbation test decisive.
4. **Composition (the L2 question).** The curvature channel crosses — do *two* self-maintaining curvature structures compose? This is the sim05 L2 question reopened with a non-saturating stigmergic glue — the direct test of H1/H10.
