...
**Status:** Refined ×10. **The action-based property is the primary load-bearing variable for the crossing; non-saturating is a secondary stability contributor.** Session 20 isolated the recruit and limit halves (recruit = necessary + almost-sufficient; limit = stability amplifier). Session 21 disentangled H11's confound (action-based vs non-saturating) via a saturating-action control: the same curvature routing with a saturating response `p = base + gain·c/(1+|c|)` instead of linear `p = base + gain·c`. The saturating action still crosses in 8/8 seeds and is stable in 6/8 (linear is 7/8); the limit half rescues both to 4/4 at d=1. Saturation costs ~0.05 in mean hold rate at d=0 (0.91→0.86) but does not collapse the crossing — criterion 3 (deposits_on_convex_fraction) holds 1.00 for both forms; only the mass-plateau gate (criterion 2p) flickers more under saturation. The non-saturating property matters for *stability*, not for the *crossing verdict*.
...
**Status:** Directionally confirmed (4/4), causally supported with a control arm, mechanism-decomposed, and now confound-resolved (Session 21). The corrected detector fires the crossing in the curvature channel and not in the baseline-pheromone control (same detector, 0/3) — H11's channel distinction is the causal variable separating crossing from non-crossing. Session 20's 2×2 factorial isolated the halves: recruit = necessary + almost-sufficient; limit = stability amplifier. Session 21's saturating-action control resolved the action-based vs non-saturating confound: the saturating action (`p = base + gain·c/(1+|c|)`) still crosses in 8/8 seeds and is stable in 6/8 (linear 7/8); the limit half rescues both to 4/4 at d=1. **Action-based routing is the primary load-bearing property; non-saturating is a secondary stability amplifier** (mean hold drops 0.91→0.86 at d=0; criterion 3 holds 1.00 for both forms — saturation slows mass equilibration, not spatial selectivity). H11's strict "non-saturating" claim is partially weakened: a saturating action-based channel still crosses stably, but less robustly. The "self-defeating" language overstates the saturating action's failure; it is "self-destabilizing" at most. The cue-vs-action distinction (H11's original framing) is still supported by the baseline-pheromone control; what the test refines is the *within-action* distinction.
...
| H7: Trace→Actor Crossing | Refined ×10; action-based primary, non-saturating secondary | Session 21 saturating-action control: saturating action crosses 8/8 (stable 6/8), linear 8/8 (stable 7/8); limit rescues both to 4/4 at d=1; criterion 3 holds 1.00 for both; saturation costs ~0.05 hold | [H7](logs/H7/) |
...
| H11: Saturating Channel | Directionally confirmed (4/4); causal with control arm; confound resolved (action-based primary, non-saturating secondary) | Session 21 saturating-action control: saturating crosses 8/8 (stable 6/8), linear 8/8 (stable 7/8); criterion 3 holds 1.00 for both; H11's strict "non-saturating" partially weakened | [H11](logs/H11/) |
