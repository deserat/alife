...
**Status:** Refined (Session 36). The stigmergic feedback loop (B → movement → co-presence → B) is self-defeating: the boundary movement mode over-amplifies B (b_max 70-203 vs 30-50 for focal), fragmenting all structures (4/4 fragmented, 0/4 coexist). The persistence-formation trade-off's fourth axis is movement-wire coupling — when the movement mechanism reads the same field as deposit suppression, the positive feedback is self-defeating (sixth instance of the two-wire principle). Diffusivity mode also worse than baseline (1/4 vs 2/4). The zone mode (separate sensory channel: own-ID material for movement) broke the loop (b_max 50.2 ≈ none's 47.9) but produced 0/4 coexist — the separate wire's signal is too noisy. The trade-off's fifth axis is signal quality on the separate wire. H7 crossing 4/4 across all modes. The 1-seed control is 0/4 at all modes (l2_crossed).
...
**Status:** Refined (Session 36). The stigmergic feedback loop (B → movement → co-presence → B) is self-defeating — the two-wire principle's sixth member: deposit suppression and agent movement on separate signals. The boundary mode over-amplifies B (4/4 fragmented, 0/4 coexist) because the B field serves double duty. The focal mode succeeds because the movement target (fixed home center) is independent of B. The zone mode (separate sensory channel: own-ID material for movement) broke the loop but didn't recover composition — the two-wire principle's seventh member: a separate wire with a noisy signal doesn't recover the function. The 1-seed l2_crossed=0/4 (structural guarantee holds); l2_outcome has a new leak (1/4 "coexist" from movement-induced fragmentation). Real insects use local mechanisms with separate sensory channels; our simulation lacks those channels. Diffusivity mode also fails (1/4). H7=4/4, 1-seed 0/4 across all modes.
...
**Status:** Refined ×25. H7 crossing is preserved across ALL movement modes (focal, boundary, diffusivity, zone, none — 4/4 each). The crossing is independent of the movement MECHANISM, not just the magnitude. Whether agents navigate globally, respond to the stigmergic B field, adjust diffusivity by zone, read a separate sensory channel, or wander freely, the single-structure crossing fires identically. Max suppression threshold (0.72–0.81) holds across all boundary architectures and movement regimes. The 1-seed control is 0/4 on l2_crossed at all modes (the zone mode has a 1/4 l2_outcome leak from movement-induced fragmentation, but l2_crossed is 0/4 — the structural guarantee holds).
...
**Status:** Refined (Session 36). The zone mode (thirteenth mechanism) broke the stigmergic feedback loop (b_max 50.2 ≈ none's 47.9 vs boundary's 104.5) but produced 0/4 coexist (worse than "none" at 2/4). The two-wire principle's seventh member: a separate wire with a noisy signal doesn't recover the function. The focal mode remains the only mechanism achieving 4/4 full co-occurrence. The 1-seed l2_crossed=0/4 (structural guarantee holds); l2_outcome has a new leak (1/4 "coexist" from movement-induced fragmentation). H7 4/4 across all modes.
...
| H5: Autopoiesis | Refined (S36) | sim14 zone mode: separate sensory channel broke stigmergic feedback loop (b_max 50.2 ≈ none vs 104.5 for boundary) but 0/4 coexist; trade-off's 5th axis = signal quality on separate wire; H7 4/4; 1-seed l2_crossed 0/4 | [H5](logs/H5/) |
| H6: Multi-Scale Autopoiesis | Refined (S36) | sim14 zone mode: two-wire principle's 7th member — separate wire with noisy signal doesn't recover function; zone broke loop (b_max 50.2 ≈ none) but 0/4 coexist; focal remains gold standard; 1-seed l2_crossed 0/4, l2_outcome leak 1/4; H7 4/4 | [H6](logs/H6/) |
| H7: Trace→Actor Crossing | Refined ×25; H7=4/4 across ALL movement modes (focal, boundary, diffusivity, zone, none); crossing independent of movement MECHANISM; 1-seed l2_crossed 0/4 at all modes | Session 36 zone sweep; Session 35 local-movement; Session 34 movement; Session 33 dual; Session 32 hybrid; Session 23: φ_sat 50% accurate; Session 22: 2×2 complete | [H7](logs/H7/) |
...
| H10: Unbounded Space Insufficiency | Refined (S36) | sim14 zone mode: separate sensory channel broke loop (b_max 50.2 ≈ none) but 0/4 coexist (worse than "none" 2/4); two-wire principle #7; focal remains only 4/4 full mechanism; 1-seed l2_crossed 0/4, l2_outcome leak 1/4; H7 4/4 | [H10](logs/H10/) |
