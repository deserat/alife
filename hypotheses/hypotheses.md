...
**Status:** Refined ×15. The 2×2 is complete (Session 22), the φ_sat predictor was falsified (Session 23), the spatially-targeted recovery metric shows no targeted scar repair (Session 24), the crossing does not compose (Session 25), and **the long-range inhibitor (Turing/Gierer-Meinhardt lateral inhibition) is a weak positive** (Session 26). sim11 added a long-range inhibitor (`I = max(0, far_smoothed_material − material)` — self-cancelling: zero at structures, high in the gap) to the curvature channel. At g=0.9, 2/4 seeds show clean composition (2-seed coexist AND 1-seed does NOT) — up from 0/4 with no inhibition. But 2/4 fragment (the 1-seed control fires too), and stable_l2 shows no stable advantage (0/4 at all gains). The H7 crossing survives inhibition at all gains (h7=4/4), so the crossing and composition are separable: the inhibitor adds a boundary without breaking the L1 crossing. **The composition problem is not just missing lateral inhibition; even the textbook boundary mechanism produces only weak, non-robust partial coexistence.**
...
**Status:** Strengthened (Session 26). sim11 tested the first explicit boundary mechanism: a long-range inhibitor (Turing/Gierer-Meinhardt lateral inhibition, `I = max(0, far_smoothed_material − material)`). At g=0.9, 2/4 seeds show clean composition (2-seed coexist AND 1-seed does NOT) — up from 0/4 with no inhibition. But 2/4 fragment (the 1-seed control fires too), and stable_l2 shows no stable advantage (0/4 at all gains). **The composition problem persists even with the textbook boundary mechanism.** The "explicit composition mechanisms" prescription (H10) is only partially supported: the first candidate (lateral inhibition) helps but does not solve. The missing ingredient may be a boundary that is itself autopoietic (self-maintaining), not just a passive inhibitor field — or heterogeneous agent policies (queued-topic #79), not just a spatial field.
...
| H7: Trace→Actor Crossing | Refined ×15; long-range inhibitor is weak positive — 2/4 clean coexistence at g=0.9 (up from 0/4), but 2/4 fragment; stable_l2 0/4; h7 survives inhibition | Session 26 sim11: 2/4 clean coexist at g=0.9 (seeds 42,123); 2/4 fragment (seeds 256,999); self-cancelling inhibitor (far−local) is critical; h7=4/4 at all gains | [H7](logs/H7/) |
...
| H10: Unbounded Space Insufficiency | Strengthened (Session 26) | sim11 long-range inhibitor: 2/4 clean coexist at g=0.9 (up from 0/4), but 2/4 fragment; composition problem persists even with textbook boundary mechanism | [H10](logs/H10/) |
