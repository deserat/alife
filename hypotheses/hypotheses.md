...
**Status:** Refined (Session 28). sim12's autopoietic boundary (with memory) is more stable (4/4 vs 1/4 for the passive) and survives a 50% perturbation — supporting H5's claim that self-maintenance (memory) produces persistence. sim13 eliminated the torus leak in the co-presence signal (direct-material max filter, no x-wrapping) but the false boundaries persist (1-seed 1/4) — the cause is agent wander, not the spatial filter. The memory-specificity trade-off is a system property (agents on a torus distribute material everywhere), not a signal property. Autopoiesis is necessary for persistence but not sufficient — the boundary also needs agent-level specificity.
...
**Status:** Refined (Session 28). sim12's autopoietic boundary B self-maintains (persists through perturbation) but is not specific to two-structure interaction (1-seed control 2/4). sim13 eliminated the torus leak in the co-presence signal but the false boundaries persist (1-seed 1/4) — the specificity cannot be achieved by a spatial filter alone. The boundary's specificity requires agent-level fidelity (agents associated with distinct structures), not just signal-level specificity. Multi-scale autopoiesis needs persistence (autopoiesis) + agent-level specificity on separate wires.
...
**Status:** Refined ×17. The 2×2 is complete (Session 22), the φ_sat predictor was falsified (Session 23), the spatially-targeted recovery metric shows no targeted scar repair (Session 24), the crossing does not compose (Session 25), the long-range inhibitor is a weak positive (Session 26), the autopoietic boundary is more stable but less specific (Session 27), and **direct-material co-presence eliminates the torus leak but does not break the trade-off** (Session 28). sim13 replaced sim12's diffused-shadow co-presence with a direct-material max filter (no x-wrapping). The initial 1-seed co-presence drops to <1% of 2-seed, but the 1-seed control still fires 1/4 — agent wander on the torus deposits material in both halves, creating real co-presence. A radius sweep (8-30) reveals a breadth-specificity dimension: small radius → merges, medium → false positives, large → fragmentation (l2_crossed ≠ l2_outcome=coexist). Clean composition is 1/4 (worse than sim12's 2/4). **The H7 crossing survives all conditions (h7=4/4). The trade-off is a system property (agent wander), not a signal property (diffusion vs. direct-material). The fix requires agent fidelity, not a better spatial filter.**
...
**Status:** Strengthened (Session 28). sim13's direct-material autopoietic boundary eliminates the torus leak but produces clean composition 1/4 (worse than sim12's 2/4). The false boundaries come from agent wander, not the spatial filter. The composition problem persists across seven independent mechanisms: chemical (sim05), saturating stigmergic (sim06/sim07), non-saturating density cap (sim08), non-saturating curvature (sim10), passive lateral inhibition (sim11), autopoietic lateral inhibition (sim12), and direct-material autopoietic boundary (sim13). **The missing ingredient is agent fidelity — not a better spatial filter or a different boundary type.**
...
| H5: Autopoiesis | Refined (S28) | sim12: memory improves persistence (4/4 vs 1/4); sim13: torus leak eliminated but false boundaries persist (agent wander) | [H5](logs/H5/) |
| H6: Multi-Scale Autopoiesis | Refined (S28) | sim12: B self-maintains but not specific; sim13: specificity needs agent-level fidelity, not spatial filter | [H6](logs/H6/) |
| H7: Trace→Actor Crossing | Refined ×17; direct-material eliminates torus leak but trade-off persists — agent wander, not diffusion, causes false boundaries; clean 1/4 (worse than 2/4); h7=4/4 | Session 28 sim13: initial 1-seed co-presence <1% but 1-seed control still 1/4; radius sweep 8-30: small→merge, medium→false positive, large→fragment; l2_crossed ≠ l2_outcome | [H7](logs/H7/) |
...
| H10: Unbounded Space Insufficiency | Strengthened (Session 28) | sim13 direct-material: clean 1/4 (worse than 2/4); agent wander causes false boundaries; composition problem persists across seven mechanisms; missing ingredient is agent fidelity | [H10](logs/H10/) |
