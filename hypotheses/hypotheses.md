...
**Status:** Refined (Session 32). The hybrid suppression curve (queued-topic #99) extends H7 into the high-stability regime (g=0.9) where both proportional and decoupled lose it — the cap at g*k reduces max suppression below the crossing-killing threshold (transition between g*k=0.72 and 0.81). A new stable co-occurrence at g=0.9 (hybrid_k05 seed=123: H7=YES + coexist + stable). But the persistence-formation trade-off persists in a new form: H7+L2 vs H7+stable at g=0.9. The trade-off is about max suppression magnitude, not just curve shape. Session 31: binary gate more stable (2/4 at g=0.5) but less formation (L2 2/4); persistence-formation trade-off is curve shape. Session 30: at g=0.5, H7=4/4 and L2=4/4 co-occur with 2/4 clean but 0/4 stable. The 1-seed control is 0/4 at ALL gains in ALL modes. Autopoiesis is necessary for persistence, but the persistence-formation trade-off is about max suppression magnitude.
...
**Status:** Refined ×21. The hybrid suppression curve (Session 32: `supp = min(g*B_norm/(1+B_norm), g*k)`) PRESERVES the H7 crossing at g=0.9 where both proportional and decoupled lose it — the cap at g*k reduces max suppression below the crossing-killing threshold (transition between g*k=0.72 and 0.81). The crossing is independent of curve SHAPE at a given max suppression, but NOT independent of max suppression magnitude. A new stable co-occurrence at g=0.9 (hybrid_k05 seed=123: H7=YES + coexist + stable) — the first at the highest gain. The persistence-formation trade-off is partially broken: H7 extends to g=0.9 but stable+L2 co-occurrence remains rare (2/4 at best). The hybrid produces the most clean co-occurrences (5) of any mode. The 1-seed control is 0/4 at ALL gains in ALL modes. Session 31: H7 unchanged between proportional and decoupled modes; crossing independent of suppression curve shape. Session 30: at g=0.5, H7=4/4 and L2=4/4 co-occur with 2/4 clean but 0/4 stable. **The trade-off is about max suppression magnitude. H7 crossing depends on max supp (g*k), not gain (g) or curve shape.**
...
**Status:** Strengthened (Session 32). The hybrid suppression curve adds a tenth mechanism. The H7 crossing extends to g=0.9 via the cap at g*k (threshold between 0.72 and 0.81), and a new stable co-occurrence appears at g=0.9 (hybrid_k05 seed=123: H7=YES + coexist + stable). But the full co-occurrence ceiling (H7 + L2 + stable + clean) remains 2/4 — the composition problem persists. The trade-off is about max suppression magnitude, not curve shape. The hybrid produces the most clean co-occurrences (5, hybrid_k08) but stable rate is 1/5. Session 31: binary gates more stable (2/4→4/4 at g=0.9) but less formation (L2 4/4→2/4); trade-off is gradient vs binary. The 1-seed control is 0/4 at ALL gains in ALL modes. **The missing ingredient is not just a better curve shape — it is a mechanism that decouples max suppression from the persistence-formation tension entirely.**
...
| H5: Autopoiesis | Refined (S32) | sim12: memory improves persistence (4/4 vs 1/4); sim14: ID tagging breaks false positives (0/4); inh_gain sweep: at g=0.5 H7=4/4+L2=4/4 co-occur but 0/4 stable; decoupled sweep: binary gate more stable (2/4 at g=0.5) but less formation (L2 2/4); hybrid sweep: H7 extends to g=0.9 (cap at g*k); new stable co-occurrence at g=0.9; trade-off is about max suppression magnitude | [H5](logs/H5/) |
| H6: Multi-Scale Autopoiesis | Refined (S32) | sim14: ID tagging provides specificity (1-seed 0/4, B=0); inh_gain sweep: two wires partially decouple at g=0.5; decoupled sweep: structural decoupling strengthens persistence but weakens formation; hybrid sweep: combining gradient+binary on one wire partially works but formation-vs-persistence tension persists; wires may need separate B fields | [H6](logs/H6/) |
| H7: Trace→Actor Crossing | Refined ×21; hybrid cap preserves H7 at g=0.9 where both other modes lose it; crossing depends on max suppression (g*k) not gain; new stable co-occurrence at g=0.9; trade-off partially broken; 1-seed 0/4 at ALL gains in ALL modes | Session 32 hybrid sweep: hybrid_k08 H7=4/4 at g=0.9 (proportional 0/4, decoupled 0/4); hybrid_k05 g=0.9 seed=123 stable+H7+coexist; threshold between g*k=0.72 and 0.81; most clean co-occurrences (5, hybrid_k08); full co-occurrence ceiling 2/4 | [H7](logs/H7/) |
...
| H10: Unbounded Space Insufficiency | Strengthened (Session 32) | sim14 ID-tagged: first structurally clean composition (1-seed 0/4, B=0); hybrid sweep: H7 extends to g=0.9 via cap at g*k; new stable co-occurrence at g=0.9; full co-occurrence ceiling 2/4; trade-off is about max suppression magnitude; ten mechanisms tested | [H10](logs/H10/) |
