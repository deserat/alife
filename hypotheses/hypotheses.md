...
- No literature search has been run on this yet. Ant Colony Optimization tunes evaporation against exactly this pressure, so prior art very likely exists — the connection to ACO's pheromone-bounding schemes (MAX-MIN Ant System bounds τ explicitly) should be checked before claiming novelty.
...
| H11: Saturating Channel | NEW (2026-07-27) | sim06 self-maintenance and sim07 transport both fragmented monotonically (components 66–109→219–297; pillars 57→128) — negative feedback through a saturating cue field removes selectivity. Untested against non-saturating inhibition; no literature check yet |
