...
**Status:** Refined ×14. The 2×2 is complete (Session 22), the φ_sat predictor was falsified (Session 23), the spatially-targeted recovery metric shows no targeted scar repair (Session 24), and **the crossing does not compose** (Session 25). sim10's L2 test (two curvature-channel structures in adjacent regions, shared field + agent pool, one-seed control, baseline-pheromone control) found **15/16 two-seed runs MERGE into a single structure crossing the midline at the H7 crossing regime (decay=0.002).** The first L2 detector (per-region material retention) was broken — the one-seed control fired "coexist" because a single structure fills both halves. The corrected detector counts connected components lying entirely within each region (components crossing the midline are merged, counted in neither). The one-seed control now correctly fires 0/16 coexist. The offset×decay sweep (384 runs) found no composition advantage at the crossing regime (both channels merge 15/16). At higher decay, the curvature channel shows a modest stable_l2 advantage (+11/80 vs the 1-seed control) that the baseline does not (+3/80) — but the 1-seed control still fires there (16/80 coexist), so this is partial composition at best, not clean L2 emergence. **The crossing is a single-structure phenomenon; L2 composition needs a boundary or interaction mechanism the curvature channel lacks.**
...
**Status:** Strengthened (Session 25). sim10 tested L2 composition with a non-saturating stigmergic glue (the curvature channel). At the H7 crossing regime, 15/16 two-seed runs merge into a single structure; at higher erosion, apparent coexistence is fragmentation (the one-seed control fires too). The non-saturating glue composes no better than the saturating control. **The composition problem persists across chemical (AlChemy/sim05), saturating stigmergic (sim06/sim07), and non-saturating stigmergic (sim10) glues — the missing ingredient is a boundary mechanism, not the channel type.** This is consistent with H10's claim that explicit composition mechanisms are needed.
...
| H7: Trace→Actor Crossing | Refined ×14; the crossing does not compose — sim10 L2 test shows 15/16 two-seed runs merge at the crossing regime; non-saturating glue composes no better than saturating control | Session 25 sim10: 15/16 merge at crossing regime; 1-seed control 0/16 coexist; no advantage at crossing regime; modest stable_l2 advantage at higher decay (+11/80 curv vs +3/80 base) but 1-seed control fires there too | [H7](logs/H7/) |
...
| H10: Unbounded Space Insufficiency | Strengthened (Session 25) | sim10 non-saturating stigmergic glue: 15/16 merge at crossing regime, no better than saturating control; composition problem persists across all glue types | [H10](logs/H10/) |
