...
**Status:** Refined (Session 29). sim12's autopoietic boundary (with memory) is more stable (4/4 vs 1/4 for the passive) and survives a 50% perturbation — supporting H5's claim that self-maintenance (memory) produces persistence. sim13 eliminated the torus leak but the false boundaries persist (agent wander is the cause). sim14's ID-tagged agents break the false-positive mechanism — 1-seed control is 0/4 on all metrics (B_max=0.0), the first structurally clean composition. But the stronger boundary suppresses H7 crossing (0/4) and limits coexistence to 2/4. The trade-off shifts from specificity-vs-memory (Sessions 27-28) to strength-vs-growth: the boundary that prevents merging also suppresses the structure it protects. Autopoiesis is necessary for persistence, but the boundary's strength must be tuned.
...
**Status:** Refined (Session 29). sim12's autopoietic boundary B self-maintains (persists through perturbation) but is not specific to two-structure interaction (1-seed control 2/4). sim13 eliminated the torus leak but the false boundaries persist (agent wander is the cause). sim14's ID-tagged agents provide the agent-level specificity H6 requires — 1-seed control is 0/4 on all metrics (B_max=0.0). The two-wire requirement (persistence + specificity) is met: B has memory (persistence) and ID-based co-presence (specificity). But the wires are too tightly coupled — the stronger boundary suppresses H7 crossing (0/4). Multi-scale autopoiesis needs the persistence and specificity wires on truly separate channels: the boundary should grow where two IDs meet, but its strength should not be proportional to the co-presence's precision.
...
**Status:** Refined ×18. The 2×2 is complete (Session 22), the φ_sat predictor was falsified (Session 23), the spatially-targeted recovery metric shows no targeted scar repair (Session 24), the crossing does not compose (Session 25), the long-range inhibitor is a weak positive (Session 26), the autopoietic boundary is more stable but less specific (Session 27), direct-material co-presence eliminates the torus leak but does not break the trade-off (Session 28), and **ID-tagged agents break the false-positive mechanism but suppress the crossing** (Session 29). sim14 tags each agent with a structure ID (0=left, 1=right). Deposits carry the depositor's ID. Co-presence = min(dilate(material_by_id[0]), dilate(material_by_id[1])). For a single seed, all material is id=0 — co-presence is **structurally zero** (B_max=0.0 across all seeds). The 1-seed control is 0/4 on ALL metrics — the first structurally clean composition. But the stronger boundary suppresses H7 crossing (0/4, the first time the crossing is lost across all seeds). Clean composition is 2/4 (matching shadow/passive). **The trade-off shifts from specificity-vs-memory to strength-vs-growth: the boundary that enables composition kills the crossing. The crossing and composition are in tension, not just separable.**
...
**Status:** Strengthened (Session 29). sim14's ID-tagged agents achieve the first structurally clean composition — 1-seed control is 0/4 on all metrics (B_max=0.0). Clean composition is 2/4 (matching shadow/passive), but the stronger boundary suppresses H7 crossing (0/4). The composition problem persists across eight independent mechanisms: chemical (sim05), saturating stigmergic (sim06/sim07), non-saturating density cap (sim08), non-saturating curvature (sim10), passive lateral inhibition (sim11), autopoietic lateral inhibition (sim12), direct-material autopoietic boundary (sim13), and ID-tagged heterogeneous agents (sim14). **The nature of the problem has shifted: from "no mechanism prevents merging" to "every mechanism that prevents merging also suppresses growth." The missing ingredient is a mechanism that decouples boundary strength from boundary specificity.**
...
| H5: Autopoiesis | Refined (S29) | sim12: memory improves persistence (4/4 vs 1/4); sim13: torus leak eliminated but false boundaries persist (agent wander); sim14: ID tagging breaks false positives (0/4) but suppresses H7 (0/4) | [H5](logs/H5/) |
| H6: Multi-Scale Autopoiesis | Refined (S29) | sim14: ID tagging provides specificity (1-seed 0/4, B=0); two-wire requirement met but wires too tightly coupled — stronger boundary suppresses crossing | [H6](logs/H6/) |
| H7: Trace→Actor Crossing | Refined ×18; ID-tagged agents break false positives (1-seed 0/4, B=0.0) but suppress crossing (0/4); clean 2/4; crossing and composition in tension | Session 29 sim14: structural zero for 1-seed (B_max=0.0); stronger boundary kills crossing (0/4); trade-off shifts from specificity-vs-memory to strength-vs-growth | [H7](logs/H7/) |
...
| H10: Unbounded Space Insufficiency | Strengthened (Session 29) | sim14 ID-tagged: first structurally clean composition (1-seed 0/4, B=0); clean 2/4; H7 suppressed 0/4; composition problem across eight mechanisms; missing ingredient is decoupling strength from specificity | [H10](logs/H10/) |
