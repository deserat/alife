...
**Status:** Refined ×9. **The recruit half is load-bearing and almost-sufficient; the limit half is a stability amplifier.** Session 20 isolated the two halves in a 2×2 factorial (recruit ON/OFF × limit ON/OFF) with a seed-robustness pass (4 seeds). The recruit half (curvature routing: `curve_follow`, `deposit_prob_gain`, `excavate_prob_gain`) is **necessary and almost-sufficient** for a stable crossing: recruit-only (d=0) crosses stably in 3/4 seeds (hold rate 1.00 in 3, 0.65 in the borderline seed); neither (no recruit, no limit) never crosses (0/4). The limit half (biharmonic d-smoothing) alone is **never stable** (0/4 — criteria flicker, hold 0.40–0.55, because the smoothing shapes convex geometry that no agent is routed to; criterion 3 oscillates around 0.60). But the limit half is not morphology-only: recruit+limit is stable in **4/4** seeds where recruit-only is 3/4 — the borderline seed becomes fully stable (hold 1.0) when d>0 is added. So the limit half **amplifies the stability** of the recruit-driven crossing against seed variance. The Session-19 "half-supported" reading is upgraded: the limit half has a causal role (stability), not merely an aesthetic one (morphology). Session 19's corrected detector (relative-slope plateau gate replacing the unfalsifiable `|growth_rate|<0.01` mass-saturation gate) and its control arm (curvature crosses, baseline-pheromone 0/3) still stand.
...
- sim09: **crossing fires** (Session 19, corrected detector) — curvature channel crosses at every d in the tuned probe; baseline control does not. The d* sweep (100 combos) confirmed the old gate was the blocker, not the mechanism. Session 20: recruit half isolated as load-bearing + almost-sufficient; limit half as stability amplifier. Determinism verified.
...
**Status:** Directionally confirmed (4/4), causally supported with a control arm, and now mechanism-decomposed (Session 20). The corrected detector fires the crossing in the curvature channel and not in the baseline-pheromone control (same detector, 0/3) — H11's channel distinction is the causal variable separating crossing from non-crossing, not just a directional correlate of morphology. Session 20's 2×2 factorial isolates the halves: the recruit half (non-saturating curvature routing) is necessary and almost-sufficient for a stable crossing (3/4 seeds stable at d=0); the limit half (d-smoothing) alone is never stable (0/4 — criteria flicker, hold 0.40–0.55). But the limit half is a **stability amplifier**: recruit+limit is stable 4/4 where recruit-only is 3/4 — the borderline seed becomes fully stable when d>0 is added. So "recruit as well as limit" is: recruit = necessary + almost-sufficient; limit = stabilizer + morphology optimizer (causally contributes to robustness, not strictly necessary).
...
| H7: Trace→Actor Crossing | Refined ×9; recruit half load-bearing + almost-sufficient; limit half = stability amplifier | Session 20 2×2 factorial: recruit necessary + almost-sufficient (3/4 stable at d=0); limit alone never stable (0/4); recruit+limit 4/4 — limit amplifies stability; baseline control 0/3 | [H7](logs/H7/) |
...
| H11: Saturating Channel | Directionally confirmed (4/4); causal with control arm; mechanism-decomposed | Session 20 2×2: recruit = necessary + almost-sufficient; limit = stability amplifier + morphology optimizer; baseline control 0/3 | [H11](logs/H11/) |
