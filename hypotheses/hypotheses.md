...
**Status:** Refined (Session 27). sim12's autopoietic boundary (with memory) is more stable (4/4 vs 1/4 for the passive) and survives a 50% perturbation — supporting H5's claim that self-maintenance (memory) produces persistence. But its memory also creates false boundaries (1-seed control 2/4). Autopoiesis is necessary for persistence but not sufficient for a genuine L2 boundary — the boundary also needs specificity. The memory-specificity trade-off is the temporal analog of the two-wire principle (#73).
...
**Status:** Refined (Session 27). sim12's autopoietic boundary B self-maintains (persists through perturbation) but is not specific to two-structure interaction (1-seed control 2/4). Multi-scale autopoiesis requires both persistence (autopoiesis) and specificity (the boundary exists because TWO structures interact, not one). B has persistence but lacks specificity — it is an L1 actor masquerading as an L2 actor. The boundary needs both properties on separate wires.
...
**Status:** Refined ×16. The 2×2 is complete (Session 22), the φ_sat predictor was falsified (Session 23), the spatially-targeted recovery metric shows no targeted scar repair (Session 24), the crossing does not compose (Session 25), the long-range inhibitor is a weak positive (Session 26), and **the autopoietic boundary (memory + growth/decay) is more stable but less specific** (Session 27). sim12 added an autopoietic boundary field B with its own growth/decay dynamics: `B_new = B * (1 − b_decay) + b_growth * co_presence` where `co_presence = min(left_shadow, right_shadow)`. B produces stable coexistence in 4/4 seeds (vs 1/4 for the passive) and survives a 50% material-removal perturbation (B retains 91% at 100 steps). But its memory also creates false boundaries — the 1-seed control fires in 2/4 (vs 1/4 for the passive). Clean composition is 2/4 for both — the memory-specificity trade-off cancels out. **The H7 crossing survives all conditions (h7=4/4 across all modes and seeds). The composition problem persists even with an autopoietic boundary; the missing ingredient is a mechanism that combines memory with specificity.**
...
**Status:** Strengthened (Session 27). sim12 tested an autopoietic boundary — a boundary field B with its own growth/decay dynamics (memory). B produces more stable coexistence (4/4 stable vs 1/4 for the passive) but is not more specific (1-seed control 2/4 vs 1/4). Clean composition is 2/4 for both — the memory-specificity trade-off cancels out. The composition problem persists across six independent mechanisms: chemical collisions (AlChemy/sim05), saturating stigmergic (sim06/sim07), non-saturating density cap (sim08), non-saturating curvature (sim10), passive lateral inhibition (sim11), and autopoietic lateral inhibition (sim12). **The missing ingredient is not just a boundary mechanism or autopoiesis — it is a mechanism that combines memory with specificity.**
...
| H5: Autopoiesis | Refined (S27) | sim12: memory improves persistence (4/4 vs 1/4, survives perturbation) but creates false boundaries (1-seed 2/4) | [H5](logs/H5/) |
| H6: Multi-Scale Autopoiesis | Refined (S27) | sim12: B self-maintains but is not specific to two-structure interaction (1-seed 2/4) | [H6](logs/H6/) |
| H7: Trace→Actor Crossing | Refined ×16; autopoietic boundary is more stable but less specific — stable 4/4 (vs 1/4 passive), survives perturbation, but 1-seed 2/4; clean 2/4 = same; h7=4/4 | Session 27 sim12: stable 4/4, B persists 91% at 100 steps, coexistence survives perturbation; but 1-seed 2/4; clean 2/4 | [H7](logs/H7/) |
...
| H10: Unbounded Space Insufficiency | Strengthened (Session 27) | sim12 autopoietic boundary: stable 4/4 but not specific (clean 2/4 = same as passive); composition problem persists across six independent mechanisms | [H10](logs/H10/) |
